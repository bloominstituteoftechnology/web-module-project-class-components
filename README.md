...
- `<TodoList />` receives your Todos array and iterates over the list generating a new `<Todo />` for each element in the array. <!-- todo, setToDo = setState([]) / <Todo todo={todo} /> -->
- `<Todo />` is a component that takes in the `todo` data and displays the task to the screen. <!-- <p>{props.todo}</p> -->
- `<TodoForm>` will hold your input field and your `Add Todo` and `Clear Completed` buttons. <!-- Form -->
